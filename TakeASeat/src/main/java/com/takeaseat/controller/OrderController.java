...
public class OrderController {
	...
	public String getCurrentUserOrders(Model model,
									   @RequestParam(value = ORDER_STATUS, required = false, defaultValue = "Waiting")
											   String orderStatus) {
	...
	public String getCurrentUserOrdersFiltered(Model model,
											   @RequestParam(value = ORDER_STATUS, required = false, defaultValue = "Waiting")
													   String orderStatus) {
		final List<Order> orders = getOrderService().getCurrentUserOrders(orderStatus);
		model.addAttribute(ORDERS, orders);
	...
	public String getCurrentAdministratorReservations(Model model, RedirectAttributes redirectAttributes) {
		try {
	...
	public String getCurrentAdministratorReservations(Model model,
													  @RequestParam(value = ORDER_STATUS, required = false,
																	defaultValue = "Waiting") String orderStatus) {
		final List<Order> orders = getOrderService().getCurrentUserOrders(orderStatus);
		model.addAttribute(ORDERS, orders);
	...
	public String reviewRestaurant(@RequestParam(ORDER_ID) Long orderId, @RequestParam(MESSAGE) String message,
