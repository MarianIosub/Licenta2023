...
public class EmailComposerImpl implements EmailComposer {
    ...
    public SimpleMailMessage composeOrderConfirmationMessage(Order order) {
        ...
        message.setSubject("Order confirmation at " + order.getRestaurant().getName() + " on " + order.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
        message.setText(
                "Dear " + order.getUser().getName() + " " + order.getUser().getSurname() + ",\n\n" +
                        "Thank you for placing an order with us. We are pleased to confirm that your order has been received and is being processed.\n" +
                        "\n" +
                        "Order details:\n" +
                        "\n" +
                        ...
                        "Payment method: " + order.getCardPaymentNetwork() + " ending in " + order.getCardLastDigits() + " (expires " + order.getCardExpirationMonth() + "/" + order.getCardExpirationYear() + ")\n" +
                        "Transaction status: " + order.getTransactionStatus() + "\n" +
                        "We will send you another email once your order is approved or denied by " + order.getRestaurant().getName() + " administrator.\n" +
    ...
    public SimpleMailMessage composeReservationMessage(Order order) {
        ...
        message.setText("Dear " + order.getRestaurant().getName() + " Team,\n" +
                "\n" +
                "We would like to inform you that a new order has been placed at your restaurant by " + order.getUser().getName() + ". Please find the order details below:\n" +
                "\n" +
                "Order number: " + order.getId() + "\n" +
                "User: " + order.getUser().getName() + " " + order.getUser().getSurname() + " (" + order.getUser().getMail() + ")\n" +
                "Date: " + order.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + "\n" +
                "Starting hour: " + order.getStartingHour().toString().replace('.', ':') + "0\n" +
                "Ending hour: " + order.getEndingHour().toString().replace('.', ':') + "0\n" +
                "Total price: " + order.getTotalPrice() + " RON\n" +
    ...
    public SimpleMailMessage composeAcceptOrderMessage(Order order) {
        ...
        message.setText("Dear " + order.getUser().getName() + " " + order.getUser().getSurname() + ",\n\n" +
                ...
                "Restaurant is waiting for you at the specified date and time. If you have any special requests or concerns, please contact us.\n" +
                "\n" +
                "Thank you for choosing us and we hope you enjoy your meal!\n" +
                "\n" +
                "Best regards,\n" +
    ...
    public SimpleMailMessage composeRefuseOrderMessage(Order order) {
        ...
        message.setText("Dear " + order.getUser().getName() + " " + order.getUser().getSurname() + ",\n\n" +
                "\n" +
                "We regret to inform you that your reservation at " + order.getRestaurant().getName() + " has been refused. Order details:\n" +
                "\n" +
                "Order number: #" + order.getId() + "\n" +
                "Date:" + order.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + "\n" +
                "Starting hour: " + order.getStartingHour().toString().replace('.', ':') + "0\n" +
